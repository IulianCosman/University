...
void testEqual() {
    ...
    for(int i=10; i<=100; i++)
        s1.add(i); //adaugam elemente in primul set [10,100]]
    for(int i=-10; i<=5; i++)
        s2.add(i); //adaugam elemente in al doilea set [-10,5]

    assert(s1.equal(s2) == false);

    for(int i=500; i<1000; i++){
        s1.add(i);
        s2.add(i); //adaugam elemente in al ambele seturi [500, 1000)
    }

    assert(s1.equal(s2) == false);

    for(int i=10; i<=100; i++)
        s2.add(i); //adaugam elemente in al doilea set [10,100]
    for(int i=-10; i<=5; i++)
        s1.add(i); //adaugam elemente in primul set [-10,5]

    assert(s1.equal(s2) == true);

    s1.add(-500); //adaugam elementul -500 in primul set

    assert(s1.equal(s2) == false);

    s1.remove(-500); //stergem elementul -500 din primul set
